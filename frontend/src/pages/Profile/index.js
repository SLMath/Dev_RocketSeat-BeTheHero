...
export default function Profile() {
    ...
    // To execute an action everytime something specified occurs
    useEffect(() => {
        api.get('profile', {
            headers: {
                Authorization: ongId
    ...
    }, [ongId]); // [] = no condition => execute only once; good pratice to put all vars we use
        ...
        <div className="profile-container">
            ...
            <ul> {
                incidents.map(incident => (
                    <li key={incidents.id}> {/* key deve ser unico para cada */}
                        <strong>CASO:</strong>
                        <p>{incident.title}</p>
